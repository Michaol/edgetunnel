              ...
              订阅内容 =
                ...
                完整优选IP
                  .map((原始地址) => {
                    ...
                    return `${协议类型}://00000000-0000-4000-8000-000000000000@${节点地址}:${节点端口}?security=tls&type=${
                      config_JSON.传输协议
                    }&host=example.com&sni=example.com&path=${
                      encodeURIComponent(
        ...
        try {
          ...
          await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error("连接超时")), 1000)
            ),
          ]);
...
function Surge订阅配置文件热补丁(content, url, config_JSON) {
  ...
  let 输出内容 = "";
  const realSurgePath = config_JSON.启用0RTT
    ? config_JSON.PATH + "?ed=2560"
    : config_JSON.PATH;
  for (let x of 每行内容) {
    if (
...
function 批量替换域名(内容, hosts, 每组数量 = 2) {
  ...
  return 内容.replace(/example\.com/g, () => {
    if (count % 每组数量 === 0)
      currentRandomHost = 随机替换通配符(
        打乱后数组[Math.floor(count / 每组数量) % 打乱后数组.length]
      );
    count++;
...
async function 生成随机IP(request, count = 16, 指定端口 = -1) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const 序号 = (i + 1).toString().padStart(2, "0");
    randomIPs.push(
      `${ip}:${
        指定端口 === -1
  ...
  await Promise.allSettled(
    urls.map(async (url) => {
      try {
        ...
        const 预处理订阅明文内容 = isValidBase64(text)
          ? base64Decode(text)
          : text;
  ...
  if (!缓存反代IP || !缓存反代解析数组 || 缓存反代IP !== proxyIP) {
    ...
    async function DoH查询(域名, 记录类型) {
      try {
        const response = await fetch(
          `https://1.1.1.1/dns-query?name=${域名}&type=${记录类型}`,
          {
    ...
    console.log(
      `[反代解析] 解析完成 总数: ${
        ...
        .map(([ip, port], index) => `${index + 1}. ${ip}:${port}`)
        .join("\n")}`
    );
    缓存反代IP = proxyIP;
  } else
