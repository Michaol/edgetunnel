...
export default {
  async fetch(request, env, ctx) {
    ...
    const userID =
      envUUID && uuidRegex.test(envUUID)
        ...
        : [
            ...
            userIDMD5.slice(16, 20),
            userIDMD5.slice(20),
          ].join("-");
    const host = env.HOST
      ? env.HOST.toLowerCase()
          .replace(/^https?:\/\//, "")
          .split("/")[0]
          .split(":")[0]
      : url.hostname;
      ...
      if (
        ...
        if (访问路径 === "admin/init") {
          ...
          if (访问路径 === "admin/config.json") {
            ...
            try {
              ...
              ctx.waitUntil(
                请求日志记录(env, request, 访问IP, "Save_Config", config_JSON)
              );
              return new Response(
            ...
            try {
              ...
              return new Response(
                JSON.stringify({ error: "保存自定义IP失败: " + error.message }),
                {
                  status: 500,
                  headers: { "Content-Type": "application/json;charset=utf-8" },
                }
        ...
        if (url.searchParams.get("token") === 订阅TOKEN) {
          ...
          const isSubConverterRequest =
            request.headers.has("b64") ||
            request.headers.has("base64") ||
          ...
          if (订阅类型 === "mixed") {
            ...
            if (
              ...
              订阅内容 = 完整优选IP
                .map((原始地址) => {
                  ...
                  return `${协议类型}://${
                    config_JSON.UUID
                  }@${节点地址}:${节点端口}?security=tls&type=${
                    config_JSON.传输协议
                  }&host=${节点HOST}&sni=${节点HOST}&path=${
                    encodeURIComponent(
                      config_JSON.随机路径 ? 随机路径() + 节点路径 : 节点路径
                    ) + TLS分片参数
                  }&encryption=none${
                    config_JSON.跳过证书验证 ? "&allowInsecure=1" : ""
              ...
              优选订阅生成器HOST =
                优选订阅生成器HOST && !/^https?:\/\//i.test(优选订阅生成器HOST)
                  ? `https://${优选订阅生成器HOST}`
                  : 优选订阅生成器HOST;
              const 优选订阅生成器URL = `${优选订阅生成器HOST}/sub?host=example.com&${
                协议类型 === "v" + "le" + "ss" ? "uuid" : "pw"
              }=00000000-0000-4000-8000-000000000000&path=${
                encodeURIComponent(
                  config_JSON.随机路径 ? 随机路径() + 节点路径 : 节点路径
                ) + TLS分片参数
              }&type=${config_JSON.传输协议}`;
              try {
                ...
                if (response.ok)
                  订阅内容 = btoa(其他节点LINK + atob(await response.text()));
                else
                  return new Response(
                    "优选订阅生成器异常：" + response.statusText,
            ...
            }/sub?target=${订阅类型}&url=${encodeURIComponent(
              url.protocol +
                "//" +
                url.host +
            ...
            )}&emoji=${config_JSON.订阅转换配置.SUBEMOJI}&scv=${
              config_JSON.跳过证书验证
            }`;
            try {
    ...
    try {
      ...
      if (新请求头.has("Referer")) {
        const u = new URL(新请求头.get("Referer"));
        新请求头.set(
          "Referer",
          反代URL.protocol + "//" + 反代URL.host + u.pathname + u.search
        );
      }
      if (新请求头.has("Origin"))
        新请求头.set("Origin", 反代URL.protocol + "//" + 反代URL.host);
      if (!新请求头.has("User-Agent") && UA && UA !== "null")
        新请求头.set("User-Agent", UA);
      return fetch(
        new Request(
          反代URL.protocol + 反代URL.host + url.pathname + url.search,
          {
    ...
    .pipeTo(
      new WritableStream({
        async write(chunk) {
          ...
          if (判断是否是木马) {
            const { port, hostname, rawClientData } = 解析木马请求(
              chunk,
              yourUUID
            );
            ...
            const { port, hostname, rawIndex, version, isUDP } = 解析魏烈思请求(
              chunk,
              yourUUID
            );
            if (isSpeedTestSite(hostname))
    ...
    const msg =
      ...
      `${
        config_JSON.CF.Usage.success
          ? `📊 <b>请求用量：</b>${config_JSON.CF.Usage.total}/100000 <b>${(
              (config_JSON.CF.Usage.total / 100000) *
              100
            ).toFixed(2)}%</b>\n`
          : ""
      }`;
...
async function 读取config_JSON(env, hostname, userID, 重置配置 = false) {
  ...
  config_JSON.PATH = config_JSON.反代.SOCKS5.启用
    ? "/" +
      ...
      (config_JSON.反代.SOCKS5.全局 ? "://" : "=") +
      config_JSON.反代.SOCKS5.账号
    : config_JSON.反代.PROXYIP === "auto"
    ? "/"
    : `/proxyip=${config_JSON.反代.PROXYIP}`;
  const TLS分片参数 =
    config_JSON.TLS分片 == "Shadowrocket"
      ? `&fragment=${encodeURIComponent("1,40-60,30-50,tlshello")}`
      : config_JSON.TLS分片 == "Happ"
  ...
  config_JSON.LINK = `${
    config_JSON.协议类型
  }://${userID}@${host}:443?security=tls&type=${
    config_JSON.传输协议
  }&host=${host}&sni=${host}&path=${
    encodeURIComponent(
...
// 简化运营商名称（混合版：去掉AS号和公司后缀）
function 简化运营商名称(org) {
  if (!org) return "Unknown";
  return (
    org
      .replace(/^AS\d+\s+/, "") // 去掉 AS 号
      .replace(/,?\s*(Inc\.|LLC|Ltd\.|Co\.|Corporation)\.?$/i, "") // 去掉公司后缀
      .trim() || "Unknown"
  );
}

// 批量查询IP段(CIDR)的归属地信息（带KV缓存 + Batch API）
async function 批量查询CIDR归属地(cidrs, env) {
  const cidrGeoMap = {};
  const uncachedCIDRs = [];
  const uncachedIPs = [];
  const cidrToIPMap = {}; // 记录 CIDR -> 随机IP 的映射

  // 1. 检查 KV 缓存
  for (const cidr of cidrs) {
    const cacheKey = `ip_cidr_${cidr.replace(/\//g, "_")}`;
    try {
      const cached = await env.KV.get(cacheKey);
      if (cached) {
        cidrGeoMap[cidr] = JSON.parse(cached);
      } else {
        uncachedCIDRs.push(cidr);
      }
    } catch (e) {
      uncachedCIDRs.push(cidr);
    }
  ...
  if (uncachedCIDRs.length === 0) return cidrGeoMap;

  // 2. 为未缓存的 CIDR 生成随机 IP 用于查询
  for (const cidr of uncachedCIDRs) {
    ...
    uncachedIPs.push(sampleIP);
    cidrToIPMap[sampleIP] = cidr;
  }

  // 3. 调用 Batch API (http://ip-api.com/batch)
  const apiURL =
    "http://ip-api.com/batch?fields=status,message,country,city,org,isp,query";
  ...
  try {
    ...
    for (const data of results) {
      ...
      const cidr = cidrToIPMap[queryIP];
      ...
      if (data.status === "success") {
        geoInfo = {
          ...
          city: data.city || "未知",
          org: 简化运营商名称(data.org || data.isp || "Cloudflare"),
        };
      } else {
        geoInfo = { country: "未知", city: "未知", org: "Cloudflare" };
      }

      cidrGeoMap[cidr] = geoInfo;

      // 写入缓存（30天）
      const cacheKey = `ip_cidr_${cidr.replace(/\//g, "_")}`;
      env.KV.put(cacheKey, JSON.stringify(geoInfo), {
        expirationTtl: 2592000,
      }).catch(console.error);
    }
  } catch (error) {
    console.error(`Batch API Query Failed: ${error.message}`);
    // 降级处理：所有未缓存的 CIDR 标记为未知
    for (const cidr of uncachedCIDRs) {
      cidrGeoMap[cidr] = { country: "未知", city: "未知", org: "Cloudflare" };
    }
  }
...
async function 生成随机IP(request, count = 16, 指定端口 = -1, env) {
  const asnMap = { 9808: "cmcc", 4837: "cu", 4134: "ct" },
    asn = request.cf.asn;
  const cidr_url = asnMap[asn]
    ? `https://raw.githubusercontent.com/cmliu/cmliu/main/CF-CIDR/${asnMap[asn]}.txt`
    : "https://raw.githubusercontent.com/cmliu/cmliu/main/CF-CIDR.txt";
  const cfname =
    { 9808: "CF移动优选", 4837: "CF联通优选", 4134: "CF电信优选" }[asn] ||
    "CF官方优选";
  ...
  // 如果没有传入 env 或 env.KV，使用原始逻辑（向后兼容）
  if (!env || !env.KV) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const port =
      指定端口 === -1
        ? cfport[Math.floor(Math.random() * cfport.length)]
        : 指定端口;
    ipList.push({ ip, port, cidr });
  }

  // 批量查询归属地 (Batch API)
  const uniqueCIDRs = [...new Set(ipList.map((item) => item.cidr))];
  const cidrGeoMap = await 批量查询CIDR归属地(uniqueCIDRs, env);

  // 组合节点信息
  const randomIPs = ipList.map((item, index) => {
    const geo = cidrGeoMap[item.cidr] || {
      country: "未知",
      city: "未知",
      org: "Cloudflare",
    };
    // 格式：#CF电信优选(序号)-国家-城市-运营商
    const 序号 = (index + 1).toString().padStart(2, "0");
    const 备注 = `${cfname}(${序号})-${geo.country}-${geo.city}-${geo.org}`;
    return `${item.ip}:${item.port}#${备注}`;
  });
          ...
          if (
            ...
            dataLines.forEach((line) => {
              ...
              const wrappedIP = IPV6_PATTERN.test(cols[ipIdx])
                ...
                : cols[ipIdx];
              results.add(
                `${wrappedIP}:${port}#CF优选 ${cols[delayIdx]}ms ${cols[speedIdx]}MB/s`
              );
            });
          }
        }
...
async function 反代参数获取(request) {
  ...
  // 初始化
  我的SOCKS5账号 =
    searchParams.get("socks5") || searchParams.get("http") || null;
  ...
  if (searchParams.has("proxyip")) {
    ...
    const 路参IP =
      proxyMatch[1] === "proxyip." ? `proxyip.${proxyMatch[2]}` : proxyMatch[2];
    反代IP = 路参IP.includes(",")
      ? 路参IP.split(",")[Math.floor(Math.random() * 路参IP.split(",").length)]
      : 路参IP;
    return;
  ...
  if ((socksMatch = pathname.match(/\/(socks5?|http):\/?\/?(.+)/i))) {
    ...
    if (我的SOCKS5账号.includes("@")) {
      ...
      我的SOCKS5账号 = `${userPassword}@${我的SOCKS5账号.substring(
        atIndex + 1
      )}`;
    }
...
async function 获取SOCKS5账号(address) {
  ...
  const [hostPart, authPart] =
    atIndex === -1
      ? [address, undefined]
      : [address.substring(atIndex + 1), address.substring(0, atIndex)];
  ...
  if (hostPart.includes("]:")) {
    ...
    [hostname, port] = [
      hostPart.split("]:")[0] + "]",
      Number(hostPart.split("]:")[1].replace(/[^\d]/g, "")),
    ];
    ...
    [hostname, port] =
      parts.length === 2
        ? [parts[0], Number(parts[1].replace(/[^\d]/g, ""))]
        : [hostPart, 80];
  }
    ...
    const williamResult = await (async function 解析William域名(william) {
      try {
        const response = await fetch(
          `https://1.1.1.1/dns-query?name=${william}&type=TXT`,
